...
class FormCampi(BaseForm):
	...
	a_tipo_campo = SelectField('Tipo', coerce=str, validators=[DataRequired()], render_kw={'onchange': "visualizza('#a_strquery', this.value == 'combofil');"})
	...
	a_strquery = StringField('Query', validators=[Optional()], render_kw={'placeholder': 'Query SQL per il calcolo del campo', 'onload': "visualizza('#a_strquery', $('#a_tipo_campo option:selected').val() == 'combofil');"})
